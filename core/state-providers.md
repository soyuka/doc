...
`ApiPlatform\State\ParameterProviderInterface`. Since API Platform 5.0, the value left on the
parameter once the provider has run replaces the URI variable used to fetch the resource, instead of
being computed and then discarded. This lets a provider transform an identifier before the resource
lookup happens, for example to decode it:
...
[`ReadLinkParameterProvider`](filters.md#readlinkparameterprovider) (which requires
`api_platform.enable_link_security: true`). Replacing the URI variable with that resource object
would break the resource lookup, which still expects an identifier (Doctrine cannot query an entity
by another entity object). Such a provider must implement `PreservesUriVariableInterface` so the
original URI variable value is left untouched by default:
