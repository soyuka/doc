...
The `routePriority` option, available on every HTTP operation (`Get`, `GetCollection`, `Post`,
`Put`, `Patch`, `Delete`, `NotExposed`), tells the Symfony router which route to try first: **the
higher the value, the earlier the route is checked**, regardless of the order in which operations
are declared. It accepts any integer (negative values are allowed to deprioritize a route) and
