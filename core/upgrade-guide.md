...
5.0 removes long-deprecated APIs and raises the Symfony floor to `^7.4 || ^8.0`. Components ship
with a `@beta` stability flag (for example `"api-platform/state": "^5.0@beta"`) instead of `@alpha`.
If your 4.4 install runs without deprecation notices, most of this upgrade is a no-op; the sections
below cover the handful of changes that are not announced by a 4.4 deprecation.

### Breaking Changes

#### JSON:API `use_iri_as_id` Now Defaults to `false`

The [4.4 deprecation](#jsonapi-use_iri_as_id) is resolved: not setting
`api_platform.jsonapi.use_iri_as_id` explicitly now resolves to `false` instead of `true`. The
JSON:API `data.id` member carries the resource identifier and the IRI moves to `data.links.self`.
...
#### `Request::getContentType()` Fallbacks Dropped

`EntrypointAction` (GraphQL) and `SerializerContextBuilder` no longer fall back to the deprecated
`Request::getContentType()` when `Request::getContentTypeFormat()` is unavailable. This is purely
internal: the fallback existed only for Symfony versions that lacked `getContentTypeFormat()`, and
the Symfony floor is now `^7.4`, where it always exists.

**Who is affected**: only code that extends `EntrypointAction` or `SerializerContextBuilder` and
reproduces the old `method_exists(Request::class, 'getContentTypeFormat') ? ... : ...` pattern.
...
- **`DefinitionNameFactory::__construct()`'s `$distinctFormats` argument** — the constructor no
  longer accepts it. JSON Schema definition names for non-`json`/`merge-patch+json` formats are
  always suffixed with the format, unconditionally.
...
- GraphQL's `TypeConverterInterface::getType()` is renamed `convertPhpType()` and now takes a
  `Symfony\Component\TypeInfo\Type` instead of the legacy `Symfony\Component\PropertyInfo\Type`.
...
#### `CompatibleValueResolverInterface` Removed

The internal `ApiPlatform\Symfony\Bundle\ArgumentResolver\CompatibleValueResolverInterface` is
removed; `PayloadArgumentResolver` now implements Symfony's `ValueResolverInterface` directly. This
only matters if you extended or type-hinted against the removed interface.
...
[Legacy Doctrine Filters](#legacy-doctrine-filters) above for the migration path.

### Notes

- `ApiTestCase` has moved out of `api-platform/symfony` into its own `api-platform/test` package,
  required as a dev dependency. See [Testing](../symfony/testing.md) for the up-to-date setup.
