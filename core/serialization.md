...
### Doctrine Inheritance Relations (JOINED / SINGLE_TABLE)
...
abstract parent of a Doctrine `JOINED` or `SINGLE_TABLE` inheritance hierarchy, a group declared
only on a discriminator subclass used to be invisible to this check: the relation was always
serialized as an IRI, even though the concrete object returned at runtime did have a matching
property.

Since API Platform 5.0, for the Doctrine ORM and MongoDB ODM variants, the discriminator map of the
related class is also taken into account. If any subclass listed in the map declares a property in a
group that matches the current normalization or denormalization context, the relation is embedded,
as if that group had been declared directly on the parent class.
...
This is a behavior change for any existing API that relies on Doctrine `JOINED` or `SINGLE_TABLE`
inheritance where a discriminator subclass declares a group also used for normalization or
